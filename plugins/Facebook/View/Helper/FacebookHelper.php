...
class FacebookHelper extends AppHelper {
     ...
     * @access private
     */
    public $__fbShareScript = '//static.ak.fbcdn.net/connect.php/js/FB.Share';

    /**
     * Default Facebook.feed URL
     * @access private
     */
    public $__fbFeedUrl = '//www.facebook.com/dialog/feed';

    /**
     * locale, settable in the constructor
     * @link //developers.facebook.com/docs/internationalization/
     * @access public
     */
    public $locale = null;
    ...
    public function html() {
        return '<html xmlns="//www.w3.org/1999/xhtml" xmlns:fb="//ogp.me/ns/fb#">';
    }
     ...
     * - perms list: of permissions to ask for when logging in separated by commas (eg: 'email,read_stream,publish_stream'). (//developers.facebook.com/docs/authentication/permissions)
     * @param string label
    ...
    public function login($options = array(), $label = '') {
        ...
        if ((isset($options['redirect']) && $options['redirect']) || $options['custom']) {
            ...
            if ($options['img']) {
                $source = $options['img'];
                return $this->Html->link($this->Html->image($source, array(
                                    'alt' => $options['alt'],
                                    'id' => $options['id'],
                                    'onclick' => $onclick,
                                    'data-toggle' => (isset($options['data-toggle']) && !empty($options['data-toggle'])) ? $options['data-toggle'] : null,
                                    'data-content' => (isset($options['data-content']) && !empty($options['data-content'])) ? $options['data-content'] : null,
                                    'data-placement' => (isset($options['data-placement']) && !empty($options['data-placement'])) ? $options['data-placement'] : null,
                                    'class' => 'radius5px',
                                    'escape' => false)
                                ), "#", array('class' => $options['class'], 'escape' => false));
            } else {
     ...
     * @link //developers.facebook.com/docs/sharing/reference/feed-dialog
    ...
    public function feed($text = null, $options = array(), $link_options = array()) {
        ...
        $url = $this->__fbFeedUrl; //'//www.facebook.com/dialog/feed?';
    ...
    public function share($url = null, $options = array()) {
        ...
        if ($options['fbxml']) {
            ...
            $retval = $this->Html->link($options['label'], '//www.facebook.com/sharer.php', array('share_url' => $url, 'type' => $options['type'], 'name' => $options['anchor']));
     ...
     * @access public
     */
    public function picture($uid = null, $type = 'large') {
//        $options = array_merge(
//                array(
//            'uid' => $uid,
//            'facebook-logo' => 1,
...
//            return "";
//        }
        return '<img src="//graph.facebook.com/' . $uid . '/picture?type=' . $type . '">';
    }
     ...
     * Build a like box
     * $facebook->init() is required for this
     * @link //developers.facebook.com/docs/reference/plugins/like-box
