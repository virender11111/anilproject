...
class ConnectComponent extends Component {
    ...
    public function startup(Controller $controller) {
        ...
        $userDetails = $this->FB->api('/me');
        $this->Controller->Session->write(array('FB.me' => $userDetails));
        if (!$this->noAuth && !empty($this->uid)) {
            if (!empty($this->Controller->request->query['access']) && $this->Controller->request->query['access'] == 'fb_connect') {
    ...
    private function __syncFacebookUser() {
        ...
        if ($Auth->user('id')) {
            ...
            if (!$this->User->field('facebook_id')) {
                ...
                $this->User->saveField('is_facebook_connected', 1);
            }
            return true;
        } else {
            ...
            elseif (empty($this->authUser) && $this->createUser) {
                ...
                $this->authUser[$this->User->alias]['first_name'] = $userDetails['first_name'];
                $this->authUser[$this->User->alias]['last_name'] = $userDetails['last_name'];
                $this->authUser[$this->User->alias]['email'] = $userDetails['email'];
                $this->authUser[$this->User->alias]['is_facebook_connected'] = 1;
    ...
    public function friends() {
        $response = $this->FB->api('/me/friends');
        return $response;
    }

    public function friendsById($facebook_id = null) {
        if (empty($facebook_id)) {
            return null;
        }
        $response = $this->FB->api('/' . $facebook_id . '/friends');
        return $response;
    }

    public function album_photos() {
        $response = $this->FB->api('/me?fields=albums{name,photos},posts.limit(50)');
        return $response;
    }
    ...
    public function post($msg_body = array()) {
        // We have a user ID, so probably a logged in user.
        // If not, we'll get an exception, which we handle below.
        try {
            $ret_obj = $this->FB->api('/' . $this->uid . '/feed', 'POST', $msg_body);
            return $ret_obj['id'];
