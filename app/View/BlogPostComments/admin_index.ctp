...
$tableHeaders[] = $this->Paginator->sort(__('blog_post_id'));
$tableHeaders[] = $this->Paginator->sort(__('name'));
$tableHeaders[] = $this->Paginator->sort(__('email'));
$tableHeaders[] = $this->Paginator->sort(__('comment'));
...
foreach ($listAll as $listOne) {
    ...
    $row[] = $listOne['BlogPost']['title'];
    $row[] = $listOne[$modelClass]['name'];
    $row[] = $listOne[$modelClass]['email'];
    $row[] = $this->Text->truncate($listOne[$modelClass]['comment'],90,array());;
