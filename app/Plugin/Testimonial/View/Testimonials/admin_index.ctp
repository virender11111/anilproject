...
$tableHeaders[] = $this->Paginator->sort(__('image'));
...
foreach ($data as $listOne) {
    ...
    $row[] = $this->Image->img('testimonial/image/' . $listOne[$modelClass]['id'] . '/' . $listOne[$modelClass]['image'], 80, 80, array(), 'no-image.gif', false);
