...
$tableHeaders[] = $this->Paginator->sort(__('details'));
$tableHeaders[] = array($this->Paginator->sort(__('start'), 'Start Date') => array('class'=>'date-class text-center'));
$tableHeaders[] = array($this->Paginator->sort(__('end'), 'End Date') => array('class'=>'date-class text-center'));
//$tableHeaders[] = $this->Paginator->sort(__('start'));
//$tableHeaders[] = $this->Paginator->sort(__('end'));
...
$addBtn = $this->Html->link('<i class="fa fa-plus"></i> <span class="hidden-480">New Event</span>', array('action' => 'add'), array('class' => 'btn default green-haze-stripe', 'escape' => false));
...
foreach ($listAll as $listOne) {
    ...
    $row[] = $listOne[$modelClass]['details'];
   // $row[] = $listOne[$modelClass]['start'];
    $row[] = array($this->Layout->date($listOne[$modelClass]['start']), array('class'=>'text-center'));
    $row[] = array($this->Layout->date($listOne[$modelClass]['end']), array('class'=>'text-center'));
   // $row[] = $listOne[$modelClass]['end'];
    //$row[] = $this->Html->link(__('<i class="fa fa-download"></i>'), $full_path.'/'.$listOne[$modelClass]['id'].'/'.$listOne[$modelClass]['cv_upload'],array('class' => 'text-center btn btn-xs tooltips','data-placement'=>"top", 'data-original-title'=>"Download", 'title' => 'Download',  'escape' => false));
