...
class BlogPost extends AppModel {
  ...
  public $hasMany = array('BlogPostComment');
  ...
  protected function _findForView($state, $query = array(), $results = array()) {
    ...
    if ($state == 'before') {
      ...
      $query['contain'] = array(
        ...
        'BlogPostComment' => array('order' => 'id DESC')
      );
      ...
      $comments = array();
      foreach ($result['BlogPostComment'] as $k => $blogPostComment) {
        if($blogPostComment['status'] == 1) {
          $comments[$k] = $blogPostComment;
        }

      }

      $result['BlogPostComment'] = $comments;
