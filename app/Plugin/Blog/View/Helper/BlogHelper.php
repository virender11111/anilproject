  ...
  public function nav($items, $options = null) {
    ...
    foreach ($items as $item) {
      ...
      if (!empty($item['url'])) {
       ...
       // pr($item);
       if(isset($item['url']['category']) && $item['url']['category']!=''){
       	
       	$out1111 .= '<li><a href="'.Router::url('/blog/category/'.$item['url']['category'], true).'">'.$item['text'].'</a></li>';
       	//$out1111 .= '<li>'.$this->Html->link($item['text'], $item['url'], $aOptions, null, false).'</li>';
       }else{
       	$out1111 .= '<li><a href="'.Router::url('/blog/'.$item['url']['year'].'/'.$item['url']['month'], true).'">'.$item['text'].' '.@$item['url']['year'].'</a></li>';
       // $out1111 .= '<li>'.$this->Html->link($item['text'].' '.@$item['url']['year'], $item['url'], $aOptions, null, false).'</li>';
      }
      } else {
        $out .= $item['text'];
      }
