...
$this->Html->addCrumb(ucfirst('Blog Settings'), array('action' => 'index'));
$this->extend('/Common/admin_index');
$this->assign('title', $title_for_layout);
$tableHeaders[] = $this->Paginator->sort(__('id'), 'ID');
$tableHeaders[] = $this->Paginator->sort(__('setting_text'));
...
$rows = array();
foreach ($settings as $listOne) {
    $row = array();
    $row[] = $listOne[$modelClass]['id'];
    $row[] = $listOne[$modelClass]['setting_text'];
    $row[] = $listOne[$modelClass]['value'];
