 ...
 */
		...
		*/
          ...
          /*  if (count($missing_aros['users']) > 0) {
                foreach ($missing_aros['users'] as $k => $user) {
                    ...
                    // *
                    $parent_id = $this->_controller->Acl->Aro->field('id', array('model' => 'Role', 'foreign_key' => $user['User']['id']));
	...
	public function set_group_permission( $acos = array(), $roleId, $status = 0 ){
		
		$this->_controller->loadModel('Acl.AclAro');
		$this->_controller->loadModel('Acl.AclPermission');
		$this->_controller->loadModel('Acl.Aco');
