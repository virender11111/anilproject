 ...
 */
	public function admin_index() {
		...
		$acos  = $this->Acl->Aco->find('threaded', array('recursive'=> -1,'conditions'=>$conditions,'fields'=>array('id','name','parent_id'),'order'=>'order ASC'));
		
		$acosList = array();
		if( !empty($acos) ){
			foreach($acos as $aco ) {
				$acosList[$aco['Aco']['id']] = $aco['Aco']['name'];
				if( !empty($aco['children'])) {
					foreach($aco['children'] as $child ){
						$acosList[$child['Aco']['id']] = "_".$child['Aco']['name'];
					}
				}
			}
		}
		$this->set('acos', $acosList);
	}
 ...
 */
	...
	public function admin_aros( $run = true ){
		$this->AclGenerate->generate_aros($run);
		$this->redirect(array('action' => 'index'));
	}
