...
class AclController extends AclManagerAppController {
	...
	public function admin_permissions() {
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			foreach ($perms as $aco => $aros) {
				...
				foreach ($aros as $node => $perm) {
					...
					$this->Session->setFlash(__('saved1', 'Permissions'), 'message', array('class' => 'success'), 'admin');
					/*elseif ($perm == 'inherit') {
		...
		*/
		...
		$acos1 = $this->Acl->Aco->find('all', array(
				'order' => 'Aco.lft ASC', 'recursive' => 1
		));
		//$values = $this->paginate($this->Aco);
		//pr($acos1);die;
		/**************************** Customizations ***************************/
		...
		$acos=array();
		foreach ($acos1 as $key1 => $data1) {
			$aco1 =& $acos1[$key1];
				
	if (in_array($aco1['Aco']['alias'], $admin_controllers)){
				$acos[] = $data1;
				if($aco1['Aco']['parent_id'] !='' && $aco1['Aco']['parent_id'] !=null);{
					$acos2 = $this->Acl->Aco->find('all', array(
							'conditions' => array(
									'Aco.parent_id' => $aco1['Aco']['id'],
									//'alias' => $aco['Aco']['alias']
							), //array of conditions
							//'order' => 'Aco.lft ASC', 'recursive' => 1,
							'recursive' => 1
					...
					foreach($acos2 as $aco_data){
						//pr($aco_data);
						if($aco_data['Aco']['parent_id'] !='' && $aco_data['Aco']['parent_id'] !=null){
							if($aco_data['Aco']['alias'] == 'admin_index' || $aco_data['Aco']['alias'] == 'admin_edit' || $aco_data['Aco']['alias'] == 'admin_delete' || $aco_data['Aco']['alias'] == 'admin_add'){
								$acos[] = $aco_data;
							}
						}
		...
		foreach ($acos as $key => $data) {
			...
			// Generate path
			//echo $aco['Aco']['alias'];die;
			//if (in_array($aco['Aco']['alias'], $admin_controllers)){
			//echo $aco['Aco']['alias'];die;
