...
class UploadBehaviorTest extends CakeTestCase {
    ...
    public function testSimpleUpload() {
        ...
        $this->MockUpload->expects($this->once())->method('handleUploadedFile')->with(
                $this->TestUpload->alias, 'photo', $this->data['test_ok']['photo']['tmp_name'], $this->MockUpload->settings['TestUpload']['photo']['path'] . 3 . DS . $this->data['test_ok']['photo']['name']
        );
        $result = $this->TestUpload->save($this->data['test_ok']);
        $this->assertInternalType('array', $result);
        $newRecord = $this->TestUpload->findById($this->TestUpload->id);
    ...
    public function testSaveSuccessPngDefaultPathAndPathMethod() {
        ...
        $this->MockUpload->expects($this->once())
                ...
                ->with(
                        $this->equalTo('TestUploadTwo'), $this->equalTo('photo'), $this->equalTo('image-png-tmp.png'), $this->equalTo($destinationDir . 'image-png.png')
                )
        ...
        $this->MockUpload->expects($this->once())
                ...
                ->with(
                        $this->isInstanceOf('TestUploadTwo'), $this->equalTo('photo'), $this->equalTo($destinationDir), $this->equalTo($destinationDir)
                )
                ->will($this->returnValue(true));
    ...
    public function testDeleteOnUpdate() {
        ...
        $this->MockUpload->expects($this->once())->method('handleUploadedFile')->with(
                $this->TestUpload->alias, 'photo', $this->data['test_update']['photo']['tmp_name'], $this->MockUpload->settings['TestUpload']['photo']['path'] . $this->data['test_update']['id'] . DS . $this->data['test_update']['photo']['name']
        );
