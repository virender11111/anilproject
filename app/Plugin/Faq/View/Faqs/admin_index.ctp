...
$tableHeaders[] = $this->Paginator->sort(__('faq_category_id', "Category"));
...
$this->append('form-start', $this->Form->create('Faq', array(
...
foreach ($data as $listOne) {
    ...
    $row[] = $listOne['Faq']['question'];
    $row[] = array($this->Admin->status($listOne['Faq']['status']), array('class' => 'text-center'));
    $row[] = array($this->Layout->date($listOne['Faq']['created']), array('class' => 'text-center'));
